...
from ipywidgets import interact, interactive, IntSlider, widget, FloatText, FloatSlider, interact_manual
import matplotlib
import math
import warnings
warnings.filterwarnings('ignore')

def sphereresponse(aw, sigob, sigsp, thickob, depth, txheight, rxoffsetx,  rxoffsetz, dipw):
    ...
    offset_tx_rx = np.array([rxoffsetx, 0, rxoffsetz], dtype=np.int64)  # offset from transmitter to receiver array
...
def SphereWidget():
    i = interact_manual(
        ...
        rxoffsetx=FloatText(min=0.0, max=250.0, step=1, value=125.0, continuous_update=False, description="$tx-rx_x$"),

        rxoffsetz=FloatText(min=0.0, max=250.0, step=1, value=56.0, continuous_update=False, description="$tx-rx_z$"),
